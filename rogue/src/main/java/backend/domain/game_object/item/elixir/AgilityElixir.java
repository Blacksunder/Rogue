...
public class AgilityElixir extends Elixir{
    public AgilityElixir(ObjectPosition position, MapKeeper map, int current_level) {
        super(position, map, current_level);
        setStats(new CreatureStats(0, 0, 0, Constants.random(1+getQuantity(), 6+getQuantity())));
        setName("Agility Elixir");
    }
}
